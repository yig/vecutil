...
def trace_pts( pts, distances, eval_gradient, trajectories = None, dt = kDefaultTraceDt ):
    ...
    while distances.max() > 0.:
        ...
        if norms == None or not kNormalizeOnce:
...
def unit_test_distances_and_pt_gradients_to_edges():
    ...
    #print 'pts:', pts
    #print 'edges:', edges
    #print 'distances_and_pt_gradients_to_edges( pts, edges ):'
...
def test_gradients():
    ...
    import scipy
    scipy.misc.imsave( 'tmp-distances.tiff', distances )
    scipy.misc.imsave( 'tmp-grad_x.tiff', gradients_x )
    scipy.misc.imsave( 'tmp-grad_y.tiff', gradients_y )
    ...
    import scipy
    scipy.misc.imsave( 'tmp-distances-smooth.tiff', smooth_distances )
    scipy.misc.imsave( 'tmp-grad-smooth_x.tiff', smooth_gradients_x )
    scipy.misc.imsave( 'tmp-grad-smooth_y.tiff', smooth_gradients_y )
    #scipy.misc.toimage( distances.reshape( N,N ), cmin=0, cmax=255).save( 'tmp.tiff' )
...
from edge_distances import distancesSqr_to_edges, distancesSqr_to_edges_Nd, distances_and_pt_gradients_to_edges
pts = [(.5, 0.), (.5,1.), (0.,.25), (1.,.25), (0.,0.), (1.,0.), (.3,.3)]
pts = array( asarray( pts ).T )
edges = [[(.25, .25), (.75, .25)], [(.25, .25), (.25, .75)], [(.75, .25), (.75, .75)], [(.25, .75), (.75, .75)]]
'''
...
'''
from numpy import asarray, array
from edge_distances import distancesSqr_to_edges, distances_and_pt_gradients_to_edges
import random
r = random.Random( 7 )
u = r.uniform
dim = %s
...
''' % (dim, npts, nedges)
    ...
    #timeit.Timer( 'print [ abs( a - b ).sum() for a,b in zip( distances_and_pt_gradients_to_edges( pts, edges ), distances_and_pt_gradients_to_edges_Nd( pts, edges ) ) ],', setup2d, time.clock ).timeit(10)
    #print 
    ...
    print timeit.Timer( 'distancesSqr_to_edges( pts, edges )', setupNd ).repeat(4,N)
    
    print 'distances_and_pt_gradients_to_edges:',
    print timeit.Timer( 'distances_and_pt_gradients_to_edges( pts, edges )', setupNd, time.clock ).repeat(2,N)
